...
/**
 * One-time welcome intro/outro choreography for [ConfigDrivenWelcomePage].
 */
class OnboardingIntroChoreographer(
    ...
    fun restore(withDuckAi: Boolean): Boolean {
        ...
        binding.logoAnimation.alpha = 1f
        if (withDuckAi) {
    ...
    fun clearForFirstDialog(): Boolean {
        ...
        cleared = true
        if (visualsOnScreen) playOutro() else snapToOutroEndState()
    ...
    private fun startIntroAnimation(withDuckAi: Boolean, onFinished: () -> Unit) {
        ...
        introAnimatorSet = buildIntroAnimatorSet().apply {
            addListener(object : AnimatorListenerAdapter() {
                ...
                override fun onAnimationEnd(animation: Animator) {
                    if (cancelled) {
                        // The duck.ai leg is chained off this set, so once canceled nothing else will
                        // report the intro as finished.
                        if (withDuckAi) onFinished()
                        return
                    }
